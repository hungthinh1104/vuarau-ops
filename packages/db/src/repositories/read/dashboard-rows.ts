...
export async function queryRows(
  ...
  const filterClause =
    input.filter === "outstanding_delivery"
      ? sql`physical_state in ('needs_delivery', 'in_delivery') and not returned_fulfilment`
  ...
  const rowsCte = options.includeCounts
    ...
    searched_rows as (
      ...
      select searched_rows.*,
        ...
        count(*) filter (where physical_state in ('needs_delivery', 'in_delivery') and not returned_fulfilment) over() as outstanding_delivery_count,
