...
export async function queryRows(
  ...
  const activityCtes = options.includeActivity
    ? buildActivityCtes(input, saleSearchJoin, purchaseSearchJoin)
    : sql``;
  const scopedFactCtes = useFastPage ? buildScopedFactCtes() : sql``;
  const paymentCtes = buildPaymentCtes(input.workspaceId);
  const candidateCtes = useFastPage
    ? buildCandidateCtes(input, cursorClause, orderClause, limitClause)
    : sql``;
  ...
  const rows = await tx.execute(sql`
    with recursive ${searchCtes} ${paymentCtes} ${activityQueryCtes} ${candidateCtes} ${scopedFactCtes} current_return_settlements as (
      ...
      ${
        useFastPage
          ? sql`join candidate_scope latest_delivery_candidate
        on latest_delivery_candidate.kind='sale' and latest_delivery_candidate.id=d.sale_id`
          : sql``
      }
      ...
      ${
        useFastPage
          ? sql`join candidate_scope return_rollup_candidate
      ...
      ${
        useFastPage
          ? sql`join candidate_scope unresolved_return_candidate
      ...
      ${
        useFastPage
          ? sql`join candidate_scope remainder_candidate
        on remainder_candidate.kind='sale' and remainder_candidate.id=frc.sale_id`
          : sql``
      }
