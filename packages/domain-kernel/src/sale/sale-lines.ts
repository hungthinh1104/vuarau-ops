...
export function validateSaleLines(
  ...
  for (const [index, line] of lines.entries()) {
    const invalid = (problem: string) =>
      err<readonly SaleLineState[]>(
        "SALE_LINE_INVALID",
        `Sale line ${index} is invalid: ${problem}.`,
        {
          lineIndex: index,
          lineId: line.lineId,
          problem,
    ...
    if (line.unitPrice.currency !== currency) {
      return err<readonly SaleLineState[]>(
        "SALE_CURRENCY_MISMATCH",
        `Sale line ${index} is priced in ${line.unitPrice.currency} but the sale is in ${currency}.`,
        {
          lineIndex: index,
          lineId: line.lineId,
          expected: currency,
