...
| ASM-031 | What are production RPO, RTO, backup retention, encryption, and restore-drill requirements? | **operational action**    | No numeric target is assumed; production readiness remains blocked until the requirements and evidence are recorded | Deployment operator defines evidence; depot owner accepts the operational risk before production    |

Thirty-one entries: ten decided, fourteen deferred with a named trigger, seven
...
effects are recognized. ASM-030 and ASM-031 are deployment actions, not permission
to invent policy in code.
