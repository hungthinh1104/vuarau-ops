...
| ASM-042 | Which minimum, target, lead-time and reorder values define inventory risk?                       | **policy-blocked**        | `DemandObservation` now preserves source-linked customer/product/grade requests and quantities; no planning or stock-risk state is derived and current inventory reads remain source-backed quantities only                                                                             | Owner + warehouse + purchaser, before reorder or stock-risk actions ([worksheet](policy-closure-worksheet.md))                                                                    |
