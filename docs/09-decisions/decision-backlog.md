...
| ASM-039 | Which inventory valuation basis and cost timing define reproducible COGS?                        | **policy-blocked**        | `CostObservation` preserves raw source facts, but no valuation or COGS policy exists; keep cost/profit unavailable                                                                    | Owner + accountant, before valuation, COGS or margin reporting ([worksheet](policy-closure-worksheet.md))                                                     |
...
| ASM-043 | Does a stocktake need a session, variance approval and reopen path?                              | **policy-blocked**        | `ReconciliationObservation` can preserve raw expected/observed count evidence, but no stocktake session, variance approval or reopen workflow is exposed                              | Owner + warehouse, before session/variance implementation ([worksheet](policy-closure-worksheet.md))                                                          |
...
The **policy-blocked** classification applies to the derived outcome in the
question: COGS/profit, cost effects, aging/allocation, reorder risk, stocktake
...
not forbid a separately scoped raw observation when that observation has no
unapproved money, goods or management effect.
...
Supplier roles and delivery/quality observations. Such evidence is not a COGS
