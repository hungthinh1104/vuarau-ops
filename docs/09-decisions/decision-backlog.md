...
| ASM-037 | What commercial/financial consequence follows a partial customer goods return?                   | **operational action**    | `RecordDeliveryReturn` changes inventory only. V1 now supports an explicit append-only `goods_only` settlement with no money effect; debt reduction, refund, exchange value and customer credit remain blocked because those policies are not field-validated                           | Depot owner + sales/accountant + delivery/warehouse, before the first real partial return ([worksheet](cross-dimension-correction-worksheet.md))                                  |
...
| ASM-045 | What must be counted, signed off and corrected at shift or business-day close?                   | **policy-blocked**        | Narrow `observation_signoff` close/reopen is implemented with exact observations, business-day period and policy lineage; broader field authority, variance and settlement semantics remain unavailable until the field contract is approved                                            | Owner + accountant + workers, before field rollout of broader close semantics ([worksheet](policy-closure-worksheet.md))                                                          |
| ASM-046 | How are bank deposits and external statements matched to Cashbook facts?                         | **policy-blocked**        | Narrow `exact_cash_movement` match/reversal is implemented and financially neutral; unresolved-item, bank-balance, variance and settlement semantics remain unavailable until the field contract is approved                                                                            | Owner + accountant, before field rollout of broader settlement ([worksheet](policy-closure-worksheet.md))                                                                         |
