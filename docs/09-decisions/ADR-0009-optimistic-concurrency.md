...
morning. Two phones can have the same draft order open. Two people can try to
reverse the same payment.

The unacceptable outcome is a **silent** lost update: phone B confirms an order
...
1. `orders`, `payments`, and `customers` carry `version integer NOT NULL`, starting
...
| Pessimistic locking held across the user's think-time | A worker who opens an order and walks away blocks everyone. Locks must not span a human.                                                                                                                 |
...
else changed this order — here is the current version". No lock spans user
...
**Neutral.** The debt ledger needs none of this: entries are appended, never
