...
The V1 operational exception catalog is deliberately explicit. The first six
classes are derived by the Board from the listed canonical facts; stale realtime
and close readiness are separate server-authored workspace signals shown by the
shell and Operations screen, not synthetic Board rows.

| Exception                            | Source facts and detection                                                                                                                                                | Severity | Operator explanation and next action                                                                                                                                                | Resolution condition                                                                                                 |
| ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| Outstanding delivery                 | Posted Sale lines plus Delivery dispatch/return facts; `physicalState` is `needs_delivery` or `in_delivery` with quantity remaining.                                      | P1       | Goods are not physically complete. Use `Giao hàng` for an unstarted remainder or `Theo dõi giao hàng` while dispatched.                                                             | Net remaining quantity reaches zero; invalid fulfilment history becomes integrity attention instead of being hidden. |
| Returned fulfilment requiring action | Canonical Delivery Return exists while the Sale still has net fulfilment remaining; `returnedFulfilment=true`.                                                            | P1       | Goods came back before fulfilment was closed. `Xử lý hàng trả`; no inferred refund, credit or debt change.                                                                          | Later canonical fulfilment closes the remainder, or an integrity condition replaces the work item.                   |
| Incomplete receiving                 | Confirmed Purchase lines plus Receipt/inspected accepted quantities; `physicalState=needs_receiving`.                                                                     | P1       | The expected quantity is not accepted into receiving truth. `Nhận hàng`.                                                                                                            | All Purchase-line quantities are accepted by the receiving/intake facts, or the source is explicitly corrected.      |
| Unallocated customer payment         | Active Payment amount minus reversal amount minus effective allocation amount is greater than zero.                                                                       | P1       | Money was received but attribution is unresolved. `Phân bổ hoặc giữ thành tín dụng`, with the exact remaining amount.                                                               | The canonical payment is allocated or reversed to zero remaining amount.                                             |
| Overdue receivable                   | Posted Sale has an explicit `dueAt` before `asOf`, allocated amount is below Sale total, and no unallocated-payment exception takes precedence; `financialState=overdue`. | P1       | An agreed collection date has passed. `Thu tiền` when no higher-priority physical action applies; no due date is ever treated as overdue.                                           | Effective allocation covers the Sale, or the Sale is corrected/voided through its canonical command.                 |
| Reconciliation mismatch              | Workspace integrity read reports source, duplicate, projection, supplier or inventory anomalies; `status=attention`.                                                      | P0       | Canonical Money/Goods truth is not safe to close or summarize. Stop, retry the integrity read, then investigate the named source and use the owner-controlled rebuild/restore path. | Integrity returns `healthy` with zero anomaly counts after the canonical source is verified.                         |
| Stale/unavailable realtime state     | Live connection has not completed an initial/reconnect durable-feed reconciliation within its bounded window; shell state is `stale`.                                     | P1       | The screen may be old. Do not infer a new fact or resend a command. Wait for reconnect and retry the affected read.                                                                 | A live connection and durable-feed drain complete successfully, returning the state to `live`.                       |
| Operational close blocked            | Server-authored `operations.closeReadiness` is `blocked` because policy, measurable observations or the current close revision prevents close.                            | P1       | The business day cannot be signed off yet. Fix the policy, record the missing observation, or use the authorized reopen path named by the blocker.                                  | Close readiness returns `ready`, or an authorized close revision supersedes the blocking state.                      |
