...
The V1 operational exception catalog is deliberately bounded. The five conditions
below are unresolved exceptions derived from canonical facts. `outstanding_delivery`
is the source-backed delivery work condition; incomplete receiving, overdue and
awaiting payment remain useful server-authored workflow state until their own
exception producers exist. Realtime freshness and close readiness are separate
