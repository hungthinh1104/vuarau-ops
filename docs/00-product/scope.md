...
| Domain kernel         | Pure decision functions for customer, order, payment, debt                                        |
...
```
Customer → Draft order → Confirmed order → Customer debt ledger entry
        → Customer payment → Payment reversal
        → Customer debt summary → Audit history
```
...
| Full event sourcing                                              | The debt ledger is append-only; the rest of the system is not, and does not need to be ([ADR-0004](../09-decisions/ADR-0004-append-only-debt-ledger.md)) |
...
- Order lifecycle deliberately excludes payment/delivery state → those become
