...
| Purchase correction after prior Receiving lacks an approved policy or field validation (ASM-036) | Use only `commercial_replacement_only` under an approved effective workspace policy; do not fabricate reverse/re-receive movements; stop when the policy or operating contract is absent |
...
| COGS and cross-dimension cost policy remains unresolved (ASM-039/040)                            | Do not publish COGS, profit, margin or waste values; inventory value is allowed only from the approved typed adapter and is unavailable when its source truth is incomplete              |
