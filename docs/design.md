...
Authenticated operational pages use a flat sticky top bar with a structural border. The outer shell may use up to 1680px so a 240px sidebar does not compress wide work surfaces; directory/report content remains bounded at 1320px. The bar surfaces the vựa đang làm việc, the current destination on large screens, exceptional sync state (queued, blocked or offline) and an account trigger that shows the signed-in label. Theme, đổi vựa, vai trò and đăng xuất belong inside the account menu; a healthy sync state is silent.

Desktop navigation is a 248px structural sidebar that may collapse to a 72px capability rail. The collapsed rail shows one uniquely identified launcher per navigation group, not every leaf route with its label removed. A multi-route launcher opens a labelled keyboard-accessible flyout; active state uses a left structural marker instead of a decorative dot. The collapse control belongs at the bottom of the rail. The collapsed choice is local to the device. Cấu hình and Quản trị start collapsed in the expanded sidebar but remain keyboard accessible and preserve aria-current. Mobile navigation keeps the capability map stable: Hôm nay · Mua · Bán · Kho · Thêm. It must not replace those destinations with role-specific labels. When an ActionDock is mounted, mobile navigation is hidden until that dock unmounts.

The primary Product directory is the operational `Hàng hóa & kho` surface. For each Product × Hạng hàng × unit it reads server-authored physical on-hand quantity, remaining confirmed Purchase quantity, remaining posted Sale fulfilment and availability after commitments. An ungraded or not-yet-allocated inbound row stays separate and cannot cover a shortage in a named grade. Units are never converted or merged. Customer Orders and supply commitments stay outside this calculation until explicit conversion lineage prevents double counting. Catalogue aliases and lifecycle editing remain secondary to the operational stock question.

Product inventory details use four bounded tasks: `Tổng quan · Biến động · Kế hoạch · Điều chỉnh`. Current physical balance and committed-order coverage load first. Movement history, valuation, policy-backed planning and correction tools load only when their task is opened.
