...
| T-SALE-003   | Sale       | `draft` → `draft` (new lines)                   | `EditSaleDraft`          | Version matches; status is `draft`                   | none                      | `sale.draft_edited`    | `SALE_ALREADY_POSTED`, `SALE_VERSION_CONFLICT`, `SALE_LINE_INVALID`                                         | ❌ planned  |
| T-SALE-004   | Sale       | `draft` → `discarded`                           | `DiscardSaleDraft`       | Version matches; status is `draft`                   | none                      | `sale.discarded`       | `SALE_ALREADY_POSTED`, `SALE_VERSION_CONFLICT`                                                              | ❌ planned  |
...
| T-CUST-002   | Customer   | active → active (new details)                   | `UpdateCustomer`         | Version matches; name non-blank                      | none                      | `customer.updated`     | `CUSTOMER_NOT_FOUND`, `CUSTOMER_NAME_REQUIRED`, `CUSTOMER_VERSION_CONFLICT`                                 | ❌ planned  |
| T-CUST-003   | Customer   | active → inactive                               | `DeactivateCustomer`     | Version matches                                      | none                      | `customer.deactivated` | `CUSTOMER_NOT_FOUND`, `CUSTOMER_VERSION_CONFLICT`                                                           | ❌ planned  |
| T-MEMBER-001 | Membership | active → inactive                               | `RevokeMembership`       | Caller holds `workspace.manage`; not the last owner  | none                      | `membership.revoked`   | `WORKSPACE_ACCESS_DENIED`, `PERMISSION_DENIED`                                                              | ❌ planned  |
