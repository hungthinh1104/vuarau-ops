...
| `PERSISTED_NUMBER_OUT_OF_RANGE`                            | Persisted integer is outside the exact JavaScript number range; the read or command is refused without exposing its value | `field`, `requestId`                                                  | no        | BR-OPS-009                   | `INTERNAL_SERVER_ERROR` |
| `OPERATIONAL_DAY_CLOSED`                                   | Backdated command targets a business day whose operational close is still closed                                          | `businessDate`                                                        | no        | BR-OPS-010                   | `INTERNAL_SERVER_ERROR` |
| `WORKSPACE_PROFILE_BOUNDARY_CHANGE_LOCKED`                 | Business-day boundary cannot change after canonical activity exists                                                       | `workspaceId`                                                         | no        | BR-OPS-010                   | `CONFLICT`              |
...
| `SALE_CUSTOMER_INACTIVE`                                   | A new posted Sale cannot create an obligation for an inactive Customer                                                    | `saleId`, `customerId`                                                | no        | BR-SALE-019                  | `CONFLICT`              |
...
| `SALE_HAS_ACTIVE_PAYMENT_ALLOCATIONS`                      | Sale has an unreversed payment allocation and cannot be voided without orphaning attribution                              | `saleId`                                                              | no        | BR-SALE-012, BR-AGING-002    | `CONFLICT`              |
...
| `PAYMENT_ALREADY_EXISTS`                                   | A new command reused a payment identity already recorded; only the original idempotency key may replay it                 | `paymentId`                                                           | no        | BR-PAYMENT-002               | `CONFLICT`              |
...
| `PAYMENT_REVERSAL_WOULD_EXCEED_ALLOCATIONS`                | Reversal would leave active allocations larger than the payment still effective                                           | `paymentId`, `activeAllocatedAmount`, `effectivePaymentAfterReversal` | no        | BR-ACCOUNT-011               | `CONFLICT`              |
| `PAYMENT_REVERSAL_WOULD_EXCEED_CUSTOMER_CREDIT`            | Reversal would invalidate an active preserved customer-credit fact                                                        | `paymentId`, `preservedCreditAmount`, `effectivePaymentAfterReversal` | no        | BR-PAYMENT-009               | `CONFLICT`              |
...
| `PAYMENT_ALLOCATION_WOULD_EXCEED_CUSTOMER_CREDIT`          | Allocation would consume an amount already preserved as customer credit                                                   | `paymentId`, `remainingAmountMinor`                                   | no        | BR-PAYMENT-009               | `CONFLICT`              |
| `CUSTOMER_CREDIT_PRESERVATION_PAYMENT_INVALID`             | Preservation does not reference a Payment in this workspace                                                               | `paymentId`                                                           | no        | BR-PAYMENT-009               | `BAD_REQUEST`           |
| `CUSTOMER_CREDIT_PRESERVATION_AMOUNT_INVALID`              | Requested preservation amount is not positive                                                                             | —                                                                     | no        | BR-PAYMENT-009               | `BAD_REQUEST`           |
| `CUSTOMER_CREDIT_PRESERVATION_EXCEEDS_UNALLOCATED`         | Preservation exceeds the Payment amount still unallocated                                                                 | `availableAmountMinor`                                                | no        | BR-PAYMENT-009               | `CONFLICT`              |
| `CUSTOMER_CREDIT_PRESERVATION_ALREADY_RECORDED`            | Preservation identity already exists in this workspace                                                                    | `preservationId`                                                      | no        | BR-PAYMENT-009               | `CONFLICT`              |
| `CUSTOMER_CREDIT_PRESERVATION_CORRECTION_TARGET_REQUIRED`  | A correction omitted its predecessor                                                                                      | —                                                                     | no        | BR-PAYMENT-009               | `BAD_REQUEST`           |
| `CUSTOMER_CREDIT_PRESERVATION_CORRECTION_LINK_INVALID`     | A non-correction linked a predecessor                                                                                     | `relatedPreservationId`                                               | no        | BR-PAYMENT-009               | `BAD_REQUEST`           |
| `CUSTOMER_CREDIT_PRESERVATION_CORRECTION_TARGET_NOT_FOUND` | The linked preservation is outside this workspace or does not exist                                                       | `relatedPreservationId`                                               | no        | BR-PAYMENT-009               | `NOT_FOUND`             |
| `CUSTOMER_CREDIT_PRESERVATION_TARGET_ALREADY_CORRECTED`    | The linked preservation is no longer a current chain tip                                                                  | `relatedPreservationId`                                               | no        | BR-PAYMENT-009               | `CONFLICT`              |
| `CUSTOMER_CREDIT_PRESERVATION_CORRECTION_PAYMENT_MISMATCH` | Correction changed the Payment identity                                                                                   | `paymentId`, `relatedPreservationId`                                  | no        | BR-PAYMENT-009               | `BAD_REQUEST`           |
| `CUSTOMER_CREDIT_PRESERVATION_REQUIRES_FINANCIAL_COMMAND`  | A generic evidence command attempted to change Payment availability                                                       | —                                                                     | no        | BR-PAYMENT-009               | `BAD_REQUEST`           |
...
| `PRICING_RULE_INVALID`                                     | Price scope, effective range, adjustment or override reason is invalid                                                    | `productId`, `priceRuleId`                                            | no        | BR-PRICING-001               | `BAD_REQUEST`           |
...
| `IDEMPOTENCY_KEY_REUSED_WITH_DIFFERENT_COMMAND`            | Same idempotency key was used for a different command type                                                                | `idempotencyKey`                                                      | no        | BR-COMMAND-001               | `CONFLICT`              |
...
| `COMMAND_RECEIPT_RESULT_INVALID`                           | A completed idempotency receipt no longer matches the current response contract                                           | `commandType`                                                         | no        | BR-COMMAND-001               | `CONFLICT`              |
...
| `PILOT_SCOPE_EXCLUDED`                                     | Command is outside the declared shadow-pilot scope; stop and preserve the request ID                                      | `commandType`, `requestId`                                            | no        | M23 pilot gates              | `FORBIDDEN`             |
...
| `PURCHASE_PRODUCT_INACTIVE`                          | An inactive Product cannot be used for a new Purchase                                                                                                                      |
| `PURCHASE_PRODUCT_SNAPSHOT_MISMATCH`                 | Purchase line name no longer matches the selected Product                                                                                                                  |
...
| `DOCUMENT_SHARE_TOKEN_NOT_REPLAYABLE`                | A successful share-create retry cannot return the one-time plaintext token again                                                                                           |
| `REPORT_INTEGRITY_FAILURE`                           | Canonical source or projection is inconsistent                                                                                                                             |
| `WORKSPACE_POLICY_EFFECTIVE_OVERLAP`                 | Approval would create overlapping active effective windows for one policy capability                                                                                       |
...
| `OPERATIONAL_CLOSE_READINESS_BLOCKED`                | The server-side close readiness gate still has a blocker; the write-path cannot bypass the read contract                                                                   |
