...
acknowledgement with evidence and policy lineage. An acknowledgement matches only
the exact `(exceptionKind, source.kind, source.id)` of a currently unresolved
Board exception; a same-kind acknowledgement cannot satisfy another source. The
readiness read and `RecordOperationalClose` share one resolved business-day
period, policy lineage and close revision, so a write cannot use a different
temporal policy context from the read it enforces. Close currently covers all
open workspace Board work, never a paginated Board response; a date-local scope
requires an explicit policy-contract change. The readiness read exposes the
