...
Analytics candidates such as COGS, margin, aging, reorder risk, supplier score and
recommendations require an explicit workspace policy gate and source-backed
evidence. A candidate may be shown as unavailable or as raw evidence, but never as
zero, a stale projection or a recommendation when its policy is unresolved.
...
Do not present estimated profit as final profit. Unresolved analytics policy must
fail closed: never as zero, a stale projection or a recommendation.
