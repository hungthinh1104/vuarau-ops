  ...
  operations: {
    restoreBackup: async (workspaceId, payload) => {
      ...
      try {
          ...
          const row = remap({
            ...
            paymentTermsSnapshot:
              raw["paymentTermsLabel"] == null
                ? null
                : { label: raw["paymentTermsLabel"], dueAt: raw["paymentTermsDueAt"] ?? null },
            lines: commitmentLines.get(String(raw["id"])) ?? [],
          }) as unknown as SupplyCommitmentState;
          store.supplyCommitments.set(key(workspaceId, row.id), row);
        }
        ...
        for (const raw of payload.fulfilmentRemainderCases) {
          const row = remap(raw) as unknown as FulfilmentRemainderCaseDto;
          store.fulfilmentRemainderCases.set(key(workspaceId, row.id), row);
        }
