...
export const createSupplyCommitmentObservationReads = (
  store: Store,
): Pick<Repositories, "supplyCommitmentObservationReads"> => ({
  supplyCommitmentObservationReads: {
    ...
    list: async ({ workspaceId, kind, page }) => {
      const rows = [...store.supplyCommitmentObservations.values()]
