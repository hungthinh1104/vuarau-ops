...
import type { CustomerDebtSummary, LedgerEntryDraft } from "@vuarau/domain-kernel";
import { addMoney, buildDebtSummary, zeroMoney } from "@vuarau/domain-kernel";
...
export async function applyLedgerEffects(
  ...
  const appended = await repos.ledger.append(drafts);
...
async function advanceSummary(
  ...
  newEntries: readonly DebtLedgerEntryDto[],
  currency: CurrencyCode,
): Promise<void> {
  const current = await repos.debtSummaries.get(workspaceId, customerId);
  ...
  await repos.debtSummaries.save({
    workspaceId,
    customerId,
    balance,
 ...
 * disposable.
 */
export async function rebuildCustomerDebtSummary(
  repos: Repositories,
  workspaceId: WorkspaceId,
  customerId: CustomerId,
  currency: CurrencyCode,
  ...
  const entries = await repos.ledger.listByCustomer(workspaceId, customerId);
  const summary = buildDebtSummary({ workspaceId, customerId, entries, currency, updatedAt });
  await repos.debtSummaries.save(summary);
  return summary;
}
