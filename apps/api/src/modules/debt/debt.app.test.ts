...
import { createOrder } from "../order/create-order.handler.ts";
import { confirmOrder } from "../order/confirm-order.handler.ts";
...
describe("BR-DEBT-006 / TC-DEBT-002", () => {
  it("rebuilds a stale projection from the entries", async () => {
    ...
    const rebuilt = await rebuildDebtSummary(harness.deps, WORKSPACE_ID, CUSTOMER_ID);
  ...
  it("produces exactly what incremental maintenance produced", async () => {
    ...
    const incremental = harness.db.summaryFor(WORKSPACE_ID, CUSTOMER_ID);
    const rebuilt = await rebuildDebtSummary(harness.deps, WORKSPACE_ID, CUSTOMER_ID);
  ...
  it("leaves the ledger untouched while rebuilding", async () => {
    await runCasebookLedger();
    const before = [...harness.db.ledgerFor(WORKSPACE_ID, CUSTOMER_ID)];

    await rebuildDebtSummary(harness.deps, WORKSPACE_ID, CUSTOMER_ID);

    expect(harness.db.ledgerFor(WORKSPACE_ID, CUSTOMER_ID)).toEqual(before);
  });
});
