...
export function CustomerDetailController() {
  ...
  const deactivateCommand = useContractCommand(
    deactivateCustomerCommandSchema,
  ...
  const statementDocument = useContractCommand(
    generateDocumentCommandSchema,
  ...
  useEffect(() => {
    if (statementDocument.result !== null) router.push(`/documents/${statementDocument.result.id}`);
